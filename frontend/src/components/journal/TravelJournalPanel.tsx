...
Day 3: Boated across the turquoise waters of Attabad Lake and walked across the thrilling Hussaini Suspension Bridge.`;
  ...
  return (
    <div className="space-y-6 rounded-3xl border border-brand-500/30 bg-ink-900/90 p-6 backdrop-blur-2xl shadow-2xl">
      <div className="flex items-center justify-between border-b border-ink-700/60 pb-4">
        <div>
          <h3 className="flex items-center gap-2 text-lg font-bold text-slate-100">
            <BookOpen className="size-5 text-brand-400" /> AI Travel Journal & Memory Book
          </h3>
          <p className="text-xs text-slate-400">AI automatically compiles your trip notes, weather memories, and photos into an exportable storybook.</p>
        </div>
      ...
      {/* Storybook Content */}
      <div className="rounded-2xl border border-brand-500/20 bg-ink-950/80 p-5 space-y-4">
        ...
        <p className="whitespace-pre-line text-xs leading-relaxed text-slate-300 font-mono">
          {story}
        </p>
        ...
        <div className="grid grid-cols-3 gap-2 pt-2">
          {["photo-1506744038136-46273834b3fb", "photo-1512453979798-5ea266f8880c", "photo-1542051841857-5f90071e7989"].map((img, idx) => (
            <div key={idx} className="relative aspect-video rounded-xl overflow-hidden border border-ink-700">
