...
export default function SharedTripPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <header className="border-b border-slate-300/60 px-4 py-4 sm:px-6">
        <div className="mx-auto flex max-w-4xl items-center justify-between">
          <Wordmark />
          <Link to="/register" className="text-sm font-medium text-brand-400 hover:text-blue-700">
            Plan your own →
          </Link>
        </div>
      </header>
      ...
      <main className="mx-auto w-full max-w-4xl flex-1 px-4 py-8 sm:px-6">
        ...
        {shared.isError && (
          <Card className="p-8 text-center">
            <p className="text-sm text-slate-600">This share link is invalid or has been revoked.</p>
          ...
          <>
            ...
            <dl className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-3">
              {[
                ...
                <div key={label} className="rounded-lg border border-slate-300 bg-slate-100/70 px-3 py-2">
            ...
            <div className="mt-6 space-y-4">
              {shared.data.trip.itinerary.days.map((day) => (
                <Card key={day.day_number} className="overflow-hidden">
                  <div className="flex items-center justify-between border-b border-slate-300 bg-white/60 px-4 py-3">
                    <h2 className="font-medium text-slate-800">Day {day.day_number}{day.title && ` — ${day.title}`}</h2>
