...
class DemoTravelProvider(TravelProvider):
    ...
    def get_weather(self, lat: float, lon: float, days: int = 3):
            ...
            forecast.append(
                {
                    ...
                    "temp_max_c": temps[offset % len(temps)] + 2,
                    "temp_min_c": temps[offset % len(temps)] - 6,
                    "precipitation_mm": [0.0, 4.0, 0.0][offset % 3],
                    "wind_kmh": 12 + offset * 3,
                    "condition": condition,
                    "icon": icon,
                }
            )
